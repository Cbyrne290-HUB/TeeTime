    ...
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      >
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            ...
            {item.brand && (
              <p className="text-sm mt-0.5" style={{ color: "var(--text-muted)" }}>{item.brand}</p>
            )}
          </div>
          <span
      ...
      {item.listing && (
        <div
          ...
          style={{ background: "var(--surface)", borderColor: "#3b82f633" }}
        >
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="text-right">
              ...
              <p className="font-bold" style={{ color: "#22c55e" }}>
                +{fmt(item.listing.listingPrice - item.buyPrice)}
              </p>
            </div>
          </div>
        </div>
      ...
      {item.sale && profit !== null && roi !== null && (
        <div
          className="rounded-xl border p-5 mb-4"
          style={{ background: "var(--surface)", borderColor: profit >= 0 ? "#22c55e33" : "#ef444433" }}
        >
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              <div key={r.label}>
                <p className="text-xs mb-0.5" style={{ color: "var(--text-muted)" }}>{r.label}</p>
                <p className={r.bold ? "font-bold text-lg" : "font-medium text-sm"} style={r.color ? { color: r.color } : {}}>
                  {r.value}
                </p>
              </div>
            ))}
          ...
          <p className="text-xs mt-3" style={{ color: "var(--text-muted)" }}>
            ROI: <span style={{ color: roi >= 0 ? "#22c55e" : "#ef4444" }}>{roi.toFixed(1)}%</span>
            {" · "}sold {fmtDate(item.sale.saleDate)}
          </p>
        </div>
      )}
