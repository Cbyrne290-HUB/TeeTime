          ...
          <div className="space-y-3">
            {[
              ...
              <div key={r.label} className="flex justify-between items-center">
                ...
                <span className={r.bold ? "font-bold text-base" : "font-medium text-sm"} style={{ color: r.color }}>
                  {r.value}
                </span>
              </div>
            ))}
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <Link
                key={a.href}
                href={a.href}
                className="flex items-start gap-3 p-4 rounded-lg border transition-colors hover:border-purple-500"
              ...
              <div key={sale.id} className="px-5 py-3 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>
                    {categoryLabel(sale.item.category)} · {fmtDate(sale.saleDate)}
                  </p>
                </div>
                <div className="text-right">
                  <p className="font-semibold text-sm" style={{ color: sale.profit >= 0 ? "#22c55e" : "#ef4444" }}>
      ...
      {s.totalItems === 0 && (
        ...
        >
          ...
          <p className="text-sm mb-6" style={{ color: "var(--text-muted)" }}>
            Start by adding the first item you&apos;ve bought to flip.
          </p>
          <Link
            href="/inventory/add"
